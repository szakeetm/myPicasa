...
pub fn request_thumbnail(
    ...
    match generate_thumbnail(&PathBuf::from(primary_path), size) {
        Ok(Some(bytes)) => {
            ...
            let elapsed = started.elapsed().as_millis();
            info!(asset_id, elapsed_ms = elapsed, bytes = bytes.len(), "thumbnail generated");
            println!(
                "thumbnail asset_id={asset_id} generated bytes={} elapsed_ms={elapsed}",
                bytes.len()
            );
        ...
        Ok(None) => {
            let elapsed = started.elapsed().as_millis();
            info!(asset_id, elapsed_ms = elapsed, "thumbnail unavailable");
            println!("thumbnail asset_id={asset_id} unavailable elapsed_ms={elapsed}");
            Ok(None)
        }
        Err(error) => {
            error!(asset_id, %error, "thumbnail generation failed");
            println!("thumbnail asset_id={asset_id} failed error={error}");
            state
                .db
...
pub fn load_viewer_frame(asset_id: i64, state: State<AppState>) -> CommandResult<Option<String>> {
    ...
    match generate_viewer_image(&PathBuf::from(primary_path), 2400) {
        Ok(Some(bytes)) => {
            let elapsed = started.elapsed().as_millis();
            info!(asset_id, elapsed_ms = elapsed, bytes = bytes.len(), "viewer image generated");
            println!(
                "viewer asset_id={asset_id} generated bytes={} elapsed_ms={elapsed}",
                bytes.len()
            );
            Ok(Some(format!("data:image/jpeg;base64,{}", STANDARD.encode(bytes))))
        }
        Ok(None) => {
            let elapsed = started.elapsed().as_millis();
            info!(asset_id, elapsed_ms = elapsed, "viewer image unavailable");
            println!("viewer asset_id={asset_id} unavailable elapsed_ms={elapsed}");
            Ok(None)
        }
        Err(error) => {
            error!(asset_id, %error, "viewer frame load failed");
            println!("viewer asset_id={asset_id} failed error={error}");
            state
                .db
